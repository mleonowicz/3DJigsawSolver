...
def calculate_dissimilarity(
    piece_a: np.ndarray, piece_b: np.ndarray, orientation: str
) -> float:
    """
    ...
    if orientation == "LR":
        color_difference = (piece_a[:, width - 1, :, :] - piece_b[:, 0, :, :]).astype(
            "int64"
        )
    if orientation == "UD":
        color_difference = (piece_a[height - 1, :, :, :] - piece_b[0, :, :, :]).astype(
            "int64"
        )
    if orientation == "FB":
        color_difference = (piece_a[:, :, depth - 1, :] - piece_b[:, :, 0, :]).astype(
            "int64"
        )
