...
class GeneticAlgorithm(object):
    ...
    def fit(self, max_iter: int, max_no_change_iter: int = 20):
        ...
        original_puzzle = Puzzle(
            self.mapping,
            np.arange(self.mapping.num_of_puzzles, dtype=np.int32).reshape(
                self.mapping.n_pieces_x,
                self.mapping.n_pieces_y,
                self.mapping.n_pieces_z,
            ),
        )
        ...
        current_population = [Puzzle(self.mapping) for _ in range(self.population_size)]
        ...
        for t in trange(max_iter):
            ...
            # Choosing elites
            elites = list(
                sorted(current_population, key=lambda p: p.fitness)[: self.elites]
            )
            new_population = elites

            # Choose parents by the roulette method
            fitness_values = (
                current_population_fitness_values
                - current_population_fitness_values.min()
            )
            if fitness_values.sum() > 0:
                probabilities = (
                    current_population_fitness_values
                    / current_population_fitness_values.sum()
                )
            else:
                probabilities = (
                    1.0 / self.population_size * np.ones(self.population_size)
                )
            ...
            for indices in parent_indices:
                first_parent, second_parent = (
                    current_population[indices[0]],
                    current_population[indices[1]],
                )
                ...
                if np.random.uniform() < self.beta and self.mapping.n_pieces_z > 1:
                    left, right = np.random.randint(
                        low=0, high=self.mapping.n_pieces_z + 1, size=2
                    )
                    if left > right:
                        left, right = right, left
                    new_puzzle.puzzle[:, :, left:right] = new_puzzle.puzzle[
                        :, :, left:right
                    ][:, :, ::-1]
            ...
            if no_change_counter >= max_no_change_iter:
                self.logger.info(
                    f"No change for {no_change_counter} iterations. Ending prematurely."
                )
                self.save_history()
                self.draw_history()
